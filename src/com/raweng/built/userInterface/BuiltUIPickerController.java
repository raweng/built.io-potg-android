...
public class BuiltUIPickerController {
	...
	private String convertImageUriToFile(Uri imageUri, Activity activity) {
		try {
			...
			RawAppUtils.showLog(TAG, "-----------------convertImageUriToFile|" + error);
	...
	private String convertVideo(Uri cameraVideoUri, Activity activity) {
		...
		try {
			...
			RawAppUtils.showLog(TAG, "-----------------convertVideo|" + error);
			RawAppUtils.errorLog(TAG, error.toString());
		}
		return null;
	}
		...
		if(pickerRequestCode == PickerResultCode.CAPTURE_IMAGE_REQUEST_CODE.getValue()){
			...
			if (selectedImage != null) {
				...
				if (cursor != null) {
					...
					RawAppUtils.showLog(TAG, "Error reading file");
					RawAppUtils.errorLog(TAG, "Error reading file");
				}
				cursor.close();
			}
