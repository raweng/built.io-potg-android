 ...
 *		RawAppUtils.showLog("BuiltUIPickerController", error.toString());
 *	}
 *
 *  &#64;Override<br>
...
public class BuiltUIPickerController {
		...
		chooseGallery.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if (BuiltUtil.isSdPresent()) {
					...
					i.setType("image/*");
					BuiltUIPickerController.this.activity.startActivityForResult(Intent.createChooser(i, activity.getString(R.string.select_picture_text)), PickerResultCode.SELECT_IMAGE_FROM_GALLERY_REQUEST_CODE.ordinal());
				} else {
	...
	private String convertImageUriToFile(Uri imageUri, Activity activity) {
		try {
			...
			RawAppUtils.showLog("BuiltUIPickerController", "-----------------convertImageUriToFile|" + error);
		}
		return null;
	}
	...
	private String convertVideo(Uri cameraVideoUri, Activity activity) {
		...
		try {
			...
			RawAppUtils.showLog("BuiltUIPickerController", "-----------------convertVideo|" + error);
		}
		return null;
	}
		...
		if(pickerRequestCode == PickerResultCode.CAPTURE_IMAGE_REQUEST_CODE.getValue()){
			...
			if (selectedImage != null) {
				...
				if (cursor != null) {
					...
					RawAppUtils.showLog("BuiltUIPickerController", "Error reading file");
				}
				cursor.close();
			}
